...
class ModelUser {
    ...
    async postLoans(livro_idlivro = null,usuario_idusuario = null, data_emprestimo = null, data_devolucao = null) {
        ...
        this._livro_idlivro = (typeof livro_idlivro !== 'number' || livro_idlivro === null) ? this.destroy(livro_idlivro) : livro_idlivro;
        this._usuario_idusuario = (typeof usuario_idusuario !== 'number' || usuario_idusuario === null) ? this.destroy(usuario_idusuario) : usuario_idusuario;
        // Lê o arquivo de configuração 'env.json' para obter informações do banco de dados
        var envFile = JSON.parse(fs.readFileSync('./config/server/env.json', 'utf8', 'r'));
        ...
        if (result.data[0].loanCount >= ModelUser.MAX_LOANS_PER_USER) {
            throw new Error(`O usuário já possui o máximo permitido de ${ModelUser.MAX_LOANS_PER_USER} empréstimos ativos.`);
        }
